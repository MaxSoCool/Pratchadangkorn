...
// --- 1. โหลด Composer Autoload ---
// Path: จาก php/login.php ถอยไป 1 ขั้น (../) เพื่อไปหา vendor/ ที่ Root Directory
require_once __DIR__ . '/../vendor/autoload.php';

// --- 2. โหลดไฟล์ .env ---
try {
    // Path: จาก php/login.php ถอยไป 1 ขั้น (../) เพื่อไปยัง Root Directory ที่มี .env อยู่
    $dotenv = Dotenv\Dotenv::createImmutable(__DIR__ . '/../');
    $dotenv->load();
} catch (Dotenv\Exception\InvalidPathException $e) {
    // หากหาไฟล์ .env ไม่พบ ให้แสดงข้อผิดพลาดและหยุดการทำงาน
    error_log("Failed to load .env file: " . $e->getMessage());
    // คุณอาจเลือกที่จะ redirect กลับไปหน้า login พร้อม message แทน die()
    $_SESSION['login_status'] = 'failed';
    $_SESSION['login_message'] = 'เกิดข้อผิดพลาดในการโหลดการตั้งค่า: ไฟล์ .env หายไปหรือไม่ถูกต้อง';
...
// const KEY_APP = '1db2648bd3d35251c02cd33fd5080f47c24383d0cc5be27159ec8ac01a133e685'; // <<<< ลบบรรทัดนี้ออก

// --- 3. ดึงค่า KEY_APP จาก Environment Variables ---
$keyApp = $_ENV['KEY_APP'] ?? null; 

if ($keyApp === null) {
    error_log("KEY_APP not found in .env or environment variables. Please check your .env file.");
    // หากไม่พบ KEY_APP ให้แสดงข้อผิดพลาดและหยุดการทำงาน
    $_SESSION['login_status'] = 'failed';
    $_SESSION['login_message'] = 'เกิดข้อผิดพลาดในการตั้งค่า: ไม่พบ KEY_APP ในไฟล์ .env';
    header("Location: ../login-page.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $postData = [
        "keyapp" => $keyApp, // <<<< ใช้ตัวแปร $keyApp ที่ดึงมาจาก .env
        "dataset" => "ldap",
        "userid" => $username,
        "pwd" => $password
    ];
    ...
    if ($ldap_auth_passed_campus_c) {
        ...
        if ($stmt) {
            ...
            // ไม่ต้องทำ else if ($logged_in) อีกรอบ เพราะจะถูกจัดการด้านล่างอยู่แล้ว
        } else {
    ...
    if ($logged_in) {
        ...
        // เพิ่ม session สำหรับแจ้งเตือนสำเร็จ
        ...
        // กรณีเข้าสู่ระบบไม่สำเร็จจากการตรวจสอบภายใน (ถ้า LDAP ไม่ผ่านและ Fallback ก็ไม่ผ่าน)
        $_SESSION['login_status'] = 'failed';
    ...
    // กรณีที่ไม่ได้ส่งข้อมูลแบบ POST (เช่น เข้าหน้า login.php โดยตรง)
    header("Location: ../login-page.php");
    exit();
}
?>